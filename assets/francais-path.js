...
const deepLinkTarget = location.hash === '#accord-participe-passe' ? 2 : null;

if(deepLinkTarget){
  openLevel(deepLinkTarget,false);
  setTimeout(()=>{
    document.querySelector('#accord-participe-passe')?.scrollIntoView({behavior:'smooth',block:'start'});
  },120);
}else if(prior){
  openLevel(+(localStorage.getItem('billete-francais-open-level')||prior.level),false);
}else{
  $$('.fr-level-panel').forEach(p=>p.classList.remove('active'));
  $$('[data-fr-level]').forEach(b=>b.classList.remove('active'));
}

window.addEventListener('hashchange',()=>{
  if(location.hash === '#accord-participe-passe'){
    openLevel(2,false);
    setTimeout(()=>{
      document.querySelector('#accord-participe-passe')?.scrollIntoView({behavior:'smooth',block:'start'});
    },80);
